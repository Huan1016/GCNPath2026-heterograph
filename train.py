...
from model.GCNPath_Heterograph import GCNPathHeterograph

from utils.utils import *
from utils.utils_model import *
from utils.heterograph_data import (
    load_heterograph_data,
    load_heterograph_pairs,
    write_split_signature,
)
...
def parse_parameter() :
    ...
    drug_ = "processed/drug_data/GDSC_Drug_Graph.pickle"
    ic50_ = "data/heterograph_GDSC/response/IC50_GDSC_异构图.txt"
    ...
    parser.add_argument("-model", choices=["baseline", "heterograph"], default="baseline", help="Model input encoder (Default: baseline)")
    parser.add_argument("-heterograph_root", type=str, default="data/heterograph_GDSC", help="Heterograph data directory")
    parser.add_argument("-hetero_dim", type=int, default=128, help="Heterograph relation hidden dimension (Default: 128)")
...
if input_ic50.endswith("IC50_GDSC_异构图.txt") and len(ic50_data) != 89840 :
    raise ValueError("Expected 89840 response rows, found {}".format(len(ic50_data)))
...
write_split_signature(ic50_train, ic50_valid, ic50_test, args)
...
if args.model == "baseline" :
    ...
    if args.attn_mode != 0 :
        raise ValueError("Heterograph model requires -attn_mode 0")
    heterograph = load_heterograph_data(args.heterograph_root, ic50_data, args)
    train_loader = load_heterograph_pairs(ic50_train, heterograph, args, no_labels, batch_size, num_workers, shuffle=True, fix_seed=True)
    valid_loader = load_heterograph_pairs(ic50_valid, heterograph, args, no_labels, batch_size, num_workers, shuffle=False, fix_seed=True)
    test_loader = load_heterograph_pairs(ic50_test, heterograph, args, no_labels, batch_size, num_workers, shuffle=False, fix_seed=True)
...
if args.model == "baseline" :
    ...
    args_hparam = {
        "model": args.model,
        "heterograph_root": args.heterograph_root,
        "hidden_dim": args.hetero_dim,
        "dim_embed_cell": args.dim_embed_cell,
        "dim_embed_drug": args.dim_embed_drug,
        "dim_pred": args.dim_pred,
        "act": args.act,
        "drop": args.drop,
    }
    model = GCNPathHeterograph(
        graph=heterograph,
        hidden_dim=args.hetero_dim,
        dim_embed_cell=args.dim_embed_cell,
        dim_embed_drug=args.dim_embed_drug,
